...
/**
 * Represents the underlying model containing the list of projects to be displayed.
 * It is implemented using the singleton pattern, so that when the data in the model changes, all of its instances would be updated.
 * When a user applies a filter on the projects list, the model will be updated through the FilterPanel's controller.
...
public class ProjectListModel implements PropertyChangeObservable {
    ...
    private ProjectListModel() {projectList = new ArrayList<>();}

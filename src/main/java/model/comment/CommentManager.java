...
public class CommentManager extends Manager {
    ...
    public void addComment(String text, int projectId) throws NoSignedInUserException, InexistentProjectException, SQLException, InexistentDatabaseEntityException, InexistentTeamException, UnauthorisedOperationException {
        ...
        if (!teamRepository.isMemberOfTeam(team.getId(), currentUser.getId())) {
            throw new UnauthorisedOperationException(currentUser.getId(), "add comment", "they " +
                    "are not member of the team");
        }
        Comment.SavableComment comment = new Comment.SavableComment(text, projectId,
